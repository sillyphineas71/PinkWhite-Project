...
describe('DiscoveryModule (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const regRes = await request(app.getHttpServer()).post('/auth/register').send({ email: emailA, password });
    ...
    if (loginResA.status !== 200) throw new Error(`Login failed: ${JSON.stringify(loginResA.body)}`);

    const cookieHeaderA = (loginResA.headers['set-cookie'] as unknown) as string[] || [];
    const tokenCookieA = cookieHeaderA.find((c: string) => c.startsWith('access_token='));
    ...
    await request(app.getHttpServer())
      ...
      .send({ fullName: 'User A', dob: '2000-01-01T00:00:00Z', gender: 'MALE', searchGender: 'FEMALE' });
  ...
  it('/discovery/preferences (POST) - should block > 200km for free tier', async () => {
    ...
    expect(res.body.message).toBe('Khoảng cách tối đa cho gói miễn phí là 200km');
  });
  ...
  it('/discovery/visibility/toggle (POST) - should block free tier', async () => {
    const res = await getAuthReq('post', '/discovery/visibility/toggle').send({ isHidden: true });
