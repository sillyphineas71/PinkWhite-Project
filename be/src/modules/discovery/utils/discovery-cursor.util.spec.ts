...
describe('Discovery Cursor Util', () => {
  ...
  it('should throw INVALID_CURSOR on invalid base64', () => {
    expect(() => decodeDiscoveryCursor('invalid-base64-!@#')).toThrow(
      BadRequestException,
    );
    try {
  ...
  it('should throw INVALID_CURSOR on invalid JSON', () => {
    ...
    expect(() => decodeDiscoveryCursor(invalidJson)).toThrow(
      BadRequestException,
    );
  });
  ...
  it('should throw INVALID_CURSOR on missing distanceMeters', () => {
    const invalidPayload = Buffer.from(
      JSON.stringify({
        candidateUserId: 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11',
      }),
    ).toString('base64');
    expect(() => decodeDiscoveryCursor(invalidPayload)).toThrow(
      BadRequestException,
    );
  });
  ...
  it('should throw INVALID_CURSOR on missing candidateUserId', () => {
    const invalidPayload = Buffer.from(
      JSON.stringify({ distanceMeters: 100 }),
    ).toString('base64');
    expect(() => decodeDiscoveryCursor(invalidPayload)).toThrow(
      BadRequestException,
    );
  ...
  it('should throw INVALID_CURSOR on invalid candidateUserId shape', () => {
    const invalidPayload = Buffer.from(
      JSON.stringify({ distanceMeters: 100, candidateUserId: 'not-a-uuid' }),
    ).toString('base64');
    expect(() => decodeDiscoveryCursor(invalidPayload)).toThrow(
      BadRequestException,
    );
